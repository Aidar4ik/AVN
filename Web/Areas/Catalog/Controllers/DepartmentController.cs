...
namespace Web.Areas.Catalog.Controllers
{
    ...
    public class DepartmentController : BaseController<DepartmentController>
    {
        ...
        [HttpPost]
        public async Task<JsonResult> OnPostCreateOrEdit(int id, DepartmentViewModel brand)
        {
            if (ModelState.IsValid)
            {
                if (id == 0)
                {
                    var createDepartmentCommand = _mapper.Map<CreateDepartmentCommand>(brand);
                ...
                {
                    var updateBrandCommand = _mapper.Map<UpdateDepartmentCommand>(brand);
